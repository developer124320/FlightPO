...
class OLWebPage(QWebPage):
    ...
    def resolutions(self):
        if self.olResolutions is None:
            ...
            self.olResolutions = jsResolutions.toList()
...
class OpenlayersController(QObject):
    ...
    def __init__(self, parent, context, webPage, layerType):
        ...
        self.timerMax.setInterval(15000)  # TODO: different timeouts for map types
    ...
    def setup_map(self):
        ...
        temp = self.page.mainFrame().evaluateJavaScript("map.getExtent();").toMap()
        olextent = dict()
        keys = temp.keys()
        for key in keys:
            olextent[key] = temp[key].toDouble()[0]

        debug("Resulting OL extent: %s" % olextent, 3)
        if olextent is None or not hasattr(olextent, '__getitem__') or len(olextent) == 0:
            ...
            reloffset = abs((self.page.extent.yMaximum()-olextent[QString("top")])/self.page.extent.xMinimum())
    ...
    def checkMapUpdate(self):
        if self.layerType.emitsLoadEnd:
            ...
            loadEndOL = self.page.mainFrame().evaluateJavaScript("loadEnd").toBool()
